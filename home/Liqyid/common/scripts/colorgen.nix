...
let
  colorgen = pkgs.writeShellApplication {
    ...
    text = ''
        ...
        terminalscheme="$CONFIG_DIR/templates/scheme-base.json"
        ...
        if [ ! -f "$colormodefile" ]; then
            ...
            if [ "$materialscheme" = "monochrome" ]; then
              terminalscheme="$XDG_CONFIG_HOME/ags/templates/terminal/scheme-monochrome.json"
            fi
        fi
        ...
        if [[ "$1" = "#"* ]]; then # this is a color
            ...
            generate_colors_material --path "$1" \
            --mode "$lightdark" --scheme "$materialscheme" --transparency "$transparency" \
            --termscheme "$terminalscheme" --blend_bg_fg \
